-- SQL Script để tạo tài khoản test trên Supabase (FIXED VERSION)
...
-- ⚠️ BƯỚC 1: Kiểm tra và xóa tài khoản cũ (nếu có)
-- Nếu nhận được lỗi "không tồn tại", thì bỏ qua
...
-- Nếu cần XÓA TẤT CẢ:
-- DELETE FROM auth.users WHERE email IN ('admin@example.com', 'staff@example.com', 'user@example.com');
